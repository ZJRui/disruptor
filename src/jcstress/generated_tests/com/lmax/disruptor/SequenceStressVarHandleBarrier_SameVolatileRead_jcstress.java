...
import com.lmax.disruptor.SequenceStressVarHandleBarrier.SameVolatileRead;
import org.openjdk.jcstress.infra.results.JJ_Result;

public final class SequenceStressVarHandleBarrier_SameVolatileRead_jcstress extends Runner<JJ_Result> {

    volatile WorkerSync workerSync;
    SameVolatileRead[] gs;
    JJ_Result[] gr;

    public SequenceStressVarHandleBarrier_SameVolatileRead_jcstress(ForkedTestConfig config) {
        super(config);
    }

    @Override
    public void sanityCheck(Counter<JJ_Result> counter) throws Throwable {
        sanityCheck_API(counter);
        sanityCheck_Footprints(counter);
    }

    private void sanityCheck_API(Counter<JJ_Result> counter) throws Throwable {
        final SameVolatileRead s = new SameVolatileRead();
        final JJ_Result r = new JJ_Result();
    ...
    private void sanityCheck_Footprints(Counter<JJ_Result> counter) throws Throwable {
        config.adjustStrides(new FootprintEstimator() {
          public void runWith(int size, long[] cnts) {
            ...
            JJ_Result[] lr = new JJ_Result[size];
            for (int c = 0; c < size; c++) {
                SameVolatileRead s = new SameVolatileRead();
                JJ_Result r = new JJ_Result();
                lr[c] = r;
                ls[c] = s;
            }
    ...
    @Override
    public ArrayList<CounterThread<JJ_Result>> internalRun() {
        gs = new SameVolatileRead[config.minStride];
        gr = new JJ_Result[config.minStride];
        ...
        ArrayList<CounterThread<JJ_Result>> threads = new ArrayList<>(2);
        threads.add(new CounterThread<JJ_Result>() { public Counter<JJ_Result> internalRun() {
            return task_actor1();
        }});
        threads.add(new CounterThread<JJ_Result>() { public Counter<JJ_Result> internalRun() {
            return task_actor2();
    ...
    private void jcstress_consume(Counter<JJ_Result> cnt, int a) {
    ...
    private Counter<JJ_Result> task_actor1() {
        Counter<JJ_Result> counter = new Counter<>();
    ...
    private void run_actor1(SameVolatileRead[] gs, JJ_Result[] gr) {
        SameVolatileRead[] ls = gs;
        JJ_Result[] lr = gr;
        int size = ls.length;
        for (int c = 0; c < size; c++) {
    ...
    private Counter<JJ_Result> task_actor2() {
        Counter<JJ_Result> counter = new Counter<>();
    ...
    private void run_actor2(SameVolatileRead[] gs, JJ_Result[] gr) {
        SameVolatileRead[] ls = gs;
        JJ_Result[] lr = gr;
        int size = ls.length;
        for (int c = 0; c < size; c++) {
